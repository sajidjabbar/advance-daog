<!-- Header Include -->
<?php include 'inc/app.php'; ?>
<!-- Header Include -->
<!-- Css Included Here -->
<link rel="stylesheet" href="css/aog.css">


<!-- banner starts here -->
<section class="banner">
    <div class="container">
        <div class="row">
            <div class="col-lg-7">
                <div class="content-wrapper">
                    <h1 data-aos="flip-left" data-aos-easing="ease-out-cubic" data-aos-duration="2000">BASE MAINTENANCE
                    </h1>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- banner ends here -->
<!-- top text -->
<section class="aog-top-text-section">
    <div class="container">
        <div class="text-start-here">
            <h1>We Offer Wide range of Base Maintanance </h1>
            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et
                dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation.</p>
        </div>
    </div>
</section>
<!-- top  text end here -->

<section class="main-section-start">
    <div class="container">
        <div class="single-maintenance">
            <div class="row">
                <div class="col-md-6 pr-0">
                    <div class="side-img">
                        <figure>
                            <img src="img/aog-1.webp" class="img-fluid" alt="">
                        </figure>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="side-text-1">
                        <div class="normal-text">
                            <h1>Duis aute irure dolor in
                                reprehenderit</h1>
                            <p>
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor
                                incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud
                                exercitation ullamco laboris nisi ut aliquip ex ea Duis aute irure dolor in
                                reprehenderit.
                            </p>
                        </div>
                        <div class="list-items">
                            <ul>
                                <li>Lorem ipsum dolor sit amet, consectetuer adipiscing elit.</li>
                                <li>Aliquam tincidunt mauris eu risus.</li>
                            </ul>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="single-maintenance">
            <div class="row">
                <div class="col-md-6">
                    <div class="side-text-2">
                        <div class="normal-text">
                            <h1>Duis aute irure dolor in
                                reprehenderit</h1>
                            <p>
                                Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor
                                incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud
                                exercitation ullamco laboris nisi ut aliquip ex ea Duis aute irure dolor in
                                reprehenderit.
                            </p>
                        </div>
                        <div class="list-items">
                            <ul>
                                <li>Lorem ipsum dolor sit amet, consectetuer adipiscing elit.</li>
                                <li>Aliquam tincidunt mauris eu risus.</li>
                            </ul>
                        </div>
                    </div>
                </div>
                <div class="col-md-6 pl-0">
                    <div class="side-img">
                        <figure>
                            <img src="img/aog-1.webp" class="img-fluid" alt="">
                        </figure>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>

<section class="advanced-daog-qualities">
    <div class="container">
        <div class="qualities-start">
            <div class="row">
                <div class="col-md-2"></div>
                <div class="col-md-8 ">
                    <div class="text-box text-center">
                        <h1>Advanced AOG Qualities</h1>
                        <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut
                            labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation.</p>
                    </div>
                </div>
            </div>
            <div class="our-qualities">
                <div class="row">
                    <div class="col-md-4">
                        <div class="single-quality text-center">
                            <figure>
                                <img src="img/quality.webp" class="img-fluid" alt="">
                            </figure>
                            <h4>A spirit of transparency <br>
                                leads to a culture of trust.</h4>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="single-quality text-center">
                            <figure>
                                <img src="img/quality-2.webp" class="img-fluid" alt="">
                            </figure>
                            <h4>Time is our most <br>
                                precious resource.</h4>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="single-quality text-center">
                            <figure>
                                <img src="img/quality.webp" class="img-fluid" alt="">
                            </figure>
                            <h4>Understanding is at the <br>
                                heart of our business.</h4>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- End here -->

<!-- news letter section starts here -->
<?php include 'inc/newsletter.php';?>
<!-- <section class="newsletter">
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                <div class="newsletter-content-wrapper text-white">
                    <h2>Subscribe to Our Newsletter to
                        know Our latest News</h2>
                    <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut
                        labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco
                        laboris.</p>
                    <div class="input-group">
                        <input type="email" class="form-control" placeholder="Write Your Email">
                        <span class="input-group-btn">
                            <button class="btn btn-theme" type="submit">Submit</button>
                        </span>
                    </div>
                </div>
            </div>
            <div class="col-lg-4"></div>
        </div>
    </div>
</section> -->
<!-- news letter section ends here -->
<!-- Footer Include -->
<?php include 'inc/footer.php'; ?>
<!-- Footer Include -->